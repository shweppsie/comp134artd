        ...
        void DrawModel(Model model, Vector3 modelPosition, Vector3 modelRotation, float scale)
        {
            foreach (ModelMesh mesh in model.Meshes)
            {
                foreach (BasicEffect effect in mesh.Effects)
                {
                   ...
                   effect.World = Matrix.CreateScale(scale)* Matrix.CreateFromYawPitchRoll(modelRotation.Y, modelRotation.X, modelRotation.Z)*Matrix.CreateTranslation(modelPosition);
